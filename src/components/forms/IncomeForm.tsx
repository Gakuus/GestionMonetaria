...
import { incomeSchema } from '@/lib/validations/expenseSchema';

interface Props {
  categories: IncomeCategory[];
  onSubmit: (input: IncomeInput) => Promise<void>;
  onCancel: () => void;
  initial?: Partial<IncomeInput>;
}
...
export function IncomeForm({ categories, onSubmit, onCancel, initial }: Props) {
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="d-flex justify-content-end gap-2 mt-4">
        ...
        <button type="submit" className="btn btn-sm border-0 px-3 py-2 text-white d-flex align-items-center gap-1"
          disabled={loading}
          style={{ borderRadius: 10, background: 'linear-gradient(135deg, #22c55e, #16a34a)' }}>
